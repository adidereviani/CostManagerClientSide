...
function addExpense(e){
    ...
    var description = document.querySelector("#entry-description").value;
    ...
    p.then((message) => {
        console.log('The product was successfully added to report\n' + message);
    }).catch((message) => {
        console.log('Failed to add product to report\n' + message);
    })
}
...
function backupExpenses(e) {
    ...
    db.transaction(["Expenses"], "readonly").objectStore("Expenses").openCursor().onsuccess = function(e) {
        ...
        if(db) {
            ...
            expenseDescription = db.value.description;
...
function JSON2CSV(expensesDump){
    ...
    expenseDescription = new Array();

    var CSV = "expenseID,expenseValue,expenseCurrency,expenseCategory,expenseDate,expenseDescription" + "\n";
    for (var i = 0; i < expensesDump.length; i++) {
        var expenseJSON = JSON.parse(expensesDump[i]);
        var item = expenseJSON.expenseID + "," + expenseJSON.expenseValue + "," + expenseJSON.expenseCurrency + "," + expenseJSON.expenseCategory + "," + expenseJSON.expenseDate + "," + expenseJSON.expenseDescription;
        CSV += item;
    }
